...
module.exports = function(callback){
  ...
  var workingMirror;
  var n = 0;
  // TODO: might be worth using map/filter even with just two; and who
  // knows maybe they'll add more mirrors in the future
  async.parallel([
    function(callback){
      var start = Date.now();
      var httpOptions = {
    ...
    function(callback){
      ...
      if (!results[0] && !results[1]){
        return callback(
          new Error('Bad response from both mirrors'));
      }
      else if (!results[0]){
