...
const latestIdRegex = /<td>[0-9]+<\/td>/g;
let httpOptions;

module.exports = {
  id: function(mirror, callback) {
    httpOptions = { url: `${mirror}/search.php?mode=last` };
    request(httpOptions, (err, response, body) => {
      ...
      const idsResults = body.match(latestIdRegex);
  ...
  text: function(mirror,callback){
    this.id(mirror, (err, data) => {
      if (err)
        return callback(err);
      const httpOptions = { url: `${mirror}/json.php?ids=${data}&fields=*` };
      request(httpOptions, (err, response, body) => {
        if (err)
          return callback(err);
        else if (response.statusCode !== 200)
