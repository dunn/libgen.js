...
function _idFetch(options,callback){
  ...
  request(httpOpts,function(err,response,body){
    ...
    var idsResults = body.match(idsRegex);
    ...
    while (n--){
      ...
      if (!parseInt(id)){
        return callback(
          new Error('Failed to parse search results for IDs: got "' +
                   idsResults[n] + '" instead of an ID line'));
      }
      searchIds.push(id);
    }
      ...
      async.until(
        ...
        function(callback){
          ...
          request(httpOpts,function(err,response,body){
            ...
            var idsResults = body.match(idsRegex);
            // reverse the order of the results because we walk
            // through them backwards with while(n--)
            idsResults.reverse();
            var n = idsResults.length;
            while (n--){
              var id = idsResults[n].replace(/[^0-9]/g,"");
